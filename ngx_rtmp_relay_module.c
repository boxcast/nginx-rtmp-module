        ...
        ngx_rtmp_relay_create_ctx_pt create_play_ctx)
{
    ...
    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "relay: generated hash '%ui' for '%V' put in bucket '%ui'", hash, name, (hash % racf->nbuckets));
...
ngx_rtmp_relay_close(ngx_rtmp_session_t *s)
{
    ...
    if (*cctx) {
        ...
        *cctx = ctx->next;
        ctx->next = NULL; // Reset next for when ctx gets reused.
        if (*cctx && (*cctx)->next) {
